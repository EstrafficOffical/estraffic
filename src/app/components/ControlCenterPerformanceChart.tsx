...
export default function ControlCenterPerformanceChart({
  locale,
}: Props) {
      ...
      <div className="flex flex-col gap-4 border-b border-white/[0.07] px-5 py-4 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <div className="mt-1 text-xs text-white/35">
            Aggregated across every affiliate, GEO
            and active flow.
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          {[7, 30, 90].map((days) => (
            <button
              key={days}
              onClick={() =>
                setRangeDays(days as RangeDays)
              }
